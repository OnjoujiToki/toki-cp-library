  ...
  uint64_t rank(const uint64_t bit, const uint64_t pos) {
    ...
    if (bit) {
      return L[pos / LEVEL_L] + S[pos / LEVEL_S] +
             popCount(B[pos / blockBitNum] & ((1 << (pos % blockBitNum)) - 1));
    } else {
 ...
 public:
  ...
  uint64_t quantileRange(uint64_t begin_pos, uint64_t end_pos, uint64_t k) {
    ...
    for (uint64_t i = 0; i < bit_size; ++i) {
      ...
      const uint64_t num_of_zero =
          num_of_zero_end - num_of_zero_begin;  // beginからendまでにある0の数
      const uint64_t bit =
          (k < num_of_zero) ? 0 : 1;  // k番目の値の上からi番目のbitが0か1か
  ...
  std::tuple<uint64_t, uint64_t, uint64_t> rankAll(const uint64_t c,
    ...
    for (size_t i = 0; i < bit_size && begin < end; ++i) {
      ...
      if (bit) {
        rank_less_than +=
            (rank0_end - rank0_begin);  // i番目のbitが0のものは除外される
        begin = this->begin_one.at(i) + rank1_begin;
        end = this->begin_one.at(i) + rank1_end;
      } else {
        rank_more_than +=
            (rank1_end - rank1_begin);  // i番目のbitが1のものは除外される
        begin = rank0_begin;
        end = rank0_end;
      }
    }
  ...
  std::vector<std::pair<uint64_t, uint64_t>> topk(uint64_t s, uint64_t e,
    ...
    // (頻度，深さ，値)の順でソート
    auto c =
        [](const std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t>
               &l,
           const std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t>
               &r) {
          // width
    ...
    while (not que.empty()) {
      ...
      if (left0 < right0) {
        que.push(
            std::make_tuple(right0 - left0, left0, right0, depth + 1, value));
      }

      // 1
      const uint64_t left1 =
          this->begin_one.at(depth) + this->bit_arrays.at(depth).rank(1, left);
      const uint64_t right1 =
          this->begin_one.at(depth) + this->bit_arrays.at(depth).rank(1, right);
      if (left1 < right1) {
  ...
  uint64_t rangeSum(const uint64_t begin, const uint64_t end, const uint64_t x,
                    const uint64_t y) {
  ...
  std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> intersect(
    ...
    while (not que.empty()) {
      ...
      // 1
      uint64_t s1_1 =
          this->begin_one.at(depth) + this->bit_arrays.at(depth).rank(1, s1);
      uint64_t e1_1 =
          this->begin_one.at(depth) + this->bit_arrays.at(depth).rank(1, e1);
      uint64_t s2_1 =
          this->begin_one.at(depth) + this->bit_arrays.at(depth).rank(1, s2);
      uint64_t e2_1 =
          this->begin_one.at(depth) + this->bit_arrays.at(depth).rank(1, e2);
